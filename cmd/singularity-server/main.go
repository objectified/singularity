...
func initFromCmdLine() *singularity.AppConfig {
	...
	var serverBindAddr = flag.String("ServerBindAddr", "0.0.0.0", "Specify the local address that all server processes will bind to, defaults to 0.0.0.0")
	...
	appConfig.ServerBindAddr = *serverBindAddr
...
func main() {
	...
	// Start DNS server
	dnsServerPort := 53
	dnsServer := &dns.Server{Addr: appConfig.ServerBindAddr + ":" + strconv.Itoa(dnsServerPort), Net: "udp"}
